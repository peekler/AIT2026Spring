package hu.bme.aut.nfcreadwritedemo
...
class MainActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        nfcPendingIntent = PendingIntent.getActivity(this, 0, Intent(this,
            javaClass).addFlags(Intent.FLAG_ACTIVITY_SINGLE_TOP), PendingIntent.FLAG_MUTABLE)

        val btnEnableWrite = findViewById<Button>(R.id.btnEnableWrite)
    ...
    override fun onNewIntent(intent: Intent) {
        val etNfcToWrite = findViewById<EditText>(R.id.etNfcToWrite)
        if (intent.action == NfcAdapter.ACTION_TAG_DISCOVERED) {
            ...
            val record2 = NdefRecord.createUri("sms:+36301234567?body=Hello NFC BME")
            val record3 = NdefRecord.createUri("tel:+36301234567")
            val record4 = NdefRecord.createUri("http://www.bme.hu")
